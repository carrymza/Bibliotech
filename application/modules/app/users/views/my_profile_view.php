<div class="modal-dialog modal-lg" id="users" role="document">
	<div class="modal-content">
		<div class="modal-header">
			<h5 class="modal-title" id="myModalLabel"><i class="icon-people p-r-7"></i>Mi Perfil <span class="users-title">/ <?php echo $row->first_name.' '.$row->last_name;?></span></h5>
			<a href="javascript:void(0);" class="close" data-dismiss="modal" aria-hidden="true"><i class="icon-close"></i></a>
		</div>
		<div class="modal-body">
			<form role="form" id="profile-form" method="post" action="<?php echo base_url('users/update_profile/'.$row->userId);?>" class="form" enctype="multipart/form-data" onsubmit="return false;">
					...
					<div class="col-md-12">
						...
						<div class="row form-group password">
							...
							<div class="col-md-7 alpha">
								...
								<span class="text-muted fz13px">Dejar en blanco si no desea cambiar la contraseña.</span>
							</div>
		...
		<div class="modal-footer">
			<button type="button" class="btn btn-primary ladda-button" data-style="expand-left" id="user-profile-button"><span class="ladda-label">Guardar</span><span class="ladda-spinner"></span></button>
		</div>
	</div>
</div>
<script>
	$(document).ready(function () {
		var usersProfileForm = $('#profile-form').formValid({
			fields: {
				"first_name": {
					"required": true,
					"tests": [
		...
		usersProfileForm.keypress(300);

		$(document).on("click", '#user-profile-button', function(e) {
			usersProfileForm.test();
			e.preventDefault();

			if (usersProfileForm.errors() > 0){
				Ladda.stopAll();
			}else {
				var data = {type: 'post', form: '#profile-form', modal: '#modals', doAfter: 'hideModal', messageError: '.response'};
				DOM.submitData(data);
			}
		});
	});
