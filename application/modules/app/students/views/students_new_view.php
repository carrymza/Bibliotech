<div class="modal-dialog modal-lg" id="students" role="document">
	<div class="modal-content">
		<div class="modal-header">
			<h5 class="modal-title" id="myModalLabel"><i class="icon-people p-r-7"></i>Agregar Estudiante <span class="students-title"></span></h5>
			<a href="javascript:void(0);" class="close" data-dismiss="modal" aria-hidden="true"><i class="icon-close"></i></a>
		</div>
		<div class="modal-body">
			<form role="form" id="students-form" method="post" action="<?php echo base_url();?>students/insert" class="form" enctype="multipart/form-data" onsubmit="return false;">
...
<script>
	$(document).ready(function () {
		let studentsForm = $('#students').formValid({
			fields: {
				"first_name": {
					"required": true,
					"tests": [
		...
		$(document).on("click", '#students-button', function(e) {
			...
			if (studentsForm.errors() > 0){
				...
				let data = {type: 'post', form: '#students-form', modal: '#modals', doAfter: 'datatable', selector: '#students', btn: $(this), messageError: '.response', showAlert: true, titleResponse: "Exito!", textResponse: "Estudiante registrado correctamente."};
				DOM.submitData(data);
			}
		});
	});
