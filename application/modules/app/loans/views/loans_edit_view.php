<div class="modal-dialog modal-lg" id="loans" role="document">
	<div class="modal-content">
		<div class="modal-header">
			<h5 class="modal-title" id="myModalLabel"><i class="ti-package p-r-7"></i>Editar Préstamo <span class="loans-title"></span></h5>
			<a href="javascript:void(0);" class="close" data-dismiss="modal" aria-hidden="true"><i class="icon-close"></i></a>
		</div>
		<div class="modal-body">
			<form role="form" id="loans-form" method="post" action="<?php echo base_url("loans/update/".$row->loanId);?>" class="form" enctype="multipart/form-data" onsubmit="return false;">
				<div class="response"></div>
				<div>
					<input type="hidden" name="person_typeId" id="person_typeId" value="<?php echo $row->person_typeId;?>">
				</div>
				<div class="col-md-12 p-t-10">
					<div class="row form-group">
						...
						<div class="col-md-7 alpha">
							<?php echo form_dropdown_data('personId', $this->people, set_value('personId', $row->personId), "id='personId' class='form-control select2'");?>
					...
					<div class="row form-group">
						...
						<div class="col-md-5 alpha">
							<input type="text" class="form-control date" value="<?php echo $row->return_date;?>" name="return_date" id="return_date" readonly>
							<span class="valid-message"></span>
						</div>
					</div>
					<div class="row form-group">
								...
								<?php foreach ($items AS $item):?>
									<tr>
										...
										<td>
											...
											<?php echo form_dropdown_data('bookId[]', $this->books, set_value('bookId', $item->itemId), "id='bookId' class='form-control bookId select2'");?>
										</td>
										<td>
											<input type="text" class="form-control numeric" data-field="quantity" class="quantity" value="<?php echo $item->quantity;?>" name="quantity[]">
										</td>
										<td><a href="javascript:void(0);" class="remove-item" data-itemId="<?php echo $item->itemId;?>" title="Eliminar"><i class="ti-trash"></i></a></td>
									</tr>
								<?php endforeach;?>
								<?php else:?>
									<tr>
	...
	$(document).ready(function () {
		...
		$(document).on("click", '#loans-button', function(e) {
			...
			if (loansForm.errors() > 0){
				...
				let data = {type: 'post', form: '#loans-form', modal: '#modals', doAfter: 'datatable', selector: '#loans', btn: $(this), messageError: '.response', showAlert: true, titleResponse: "Exito!", textResponse: "Los datos del Prestamo han sido actualizado correctamente."};
				DOM.submitData(data);
			}
		});
