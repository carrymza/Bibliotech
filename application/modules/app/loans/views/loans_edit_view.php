<div class="modal-dialog modal-lg" id="loans" role="document">
	<div class="modal-content">
		<div class="modal-header">
			<h5 class="modal-title" id="myModalLabel"><i class="icon-people p-r-7"></i>Editar Estudiante  <span class="loans-title">/ <?php echo $row->first_name.' '.$row->last_name;?></span></h5>
			<a href="javascript:void(0);" class="close" data-dismiss="modal" aria-hidden="true"><i class="icon-close"></i></a>
		</div>
		<div class="modal-body">
			<form role="form" id="loans-form" method="post" action="<?php echo base_url('loans/update/'.$row->studentId);?>" class="form" enctype="multipart/form-data" onsubmit="return false;">
...
<script>
	$(document).ready(function () {
		var loansForm = $('#loans-form').formValid({
			fields: {
				"first_name": {
					"required": true,
					"tests": [
		...
		$(document).on("click", '#loans-button', function(e) {
			...
			if (loansForm.errors() > 0) {
				...
				var data = {type: 'post', form: '#loans-form', modal: '#modals', doAfter: 'datatable', selector: '#loans', messageError: '.response', showAlert: true, titleResponse: "Exito!", textResponse: "Datos del estudiante actualizados correctamente."};
				DOM.submitData(data);
			}
		});
	});
