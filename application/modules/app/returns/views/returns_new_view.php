<div class="modal-dialog modal-lg" id="returns" role="document">
	<div class="modal-content">
		<div class="modal-header">
			<h5 class="modal-title" id="myModalLabel"><i class="icon-people p-r-7"></i>Agregar Estudiante <span class="returns-title"></span></h5>
			<a href="javascript:void(0);" class="close" data-dismiss="modal" aria-hidden="true"><i class="icon-close"></i></a>
		</div>
		<div class="modal-body">
			<form role="form" id="returns-form" method="post" action="<?php echo base_url();?>returns/insert" class="form" enctype="multipart/form-data" onsubmit="return false;">
...
<script>
	$(document).ready(function () {
		let returnsForm = $('#returns').formValid({
			fields: {
				"first_name": {
					"required": true,
					"tests": [
		...
		$(document).on("click", '#returns-button', function(e) {
			...
			if (returnsForm.errors() > 0){
				...
				let data = {type: 'post', form: '#returns-form', modal: '#modals', doAfter: 'datatable', selector: '#returns', btn: $(this), messageError: '.response', showAlert: true, titleResponse: "Exito!", textResponse: "Estudiante registrado correctamente."};
				DOM.submitData(data);
			}
		});
	});
