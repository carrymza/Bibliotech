<div class="modal-dialog modal-lg" id="returns" role="document">
	<div class="modal-content">
		<div class="modal-header">
			<h5 class="modal-title" id="myModalLabel"><i class="icon-people p-r-7"></i>Editar Estudiante  <span class="returns-title">/ <?php echo $row->first_name.' '.$row->last_name;?></span></h5>
			<a href="javascript:void(0);" class="close" data-dismiss="modal" aria-hidden="true"><i class="icon-close"></i></a>
		</div>
		<div class="modal-body">
			<form role="form" id="returns-form" method="post" action="<?php echo base_url('returns/update/'.$row->studentId);?>" class="form" enctype="multipart/form-data" onsubmit="return false;">
		...
		<div class="modal-footer">
			<button type="button" class="btn btn-primary ladda-button" data-style="expand-left" id="returns-button"><span class="ladda-label">Guardar</span><span class="ladda-spinner"></span></button>
		</div>
	</div>
</div>
<script>
	$(document).ready(function () {
		var returnsForm = $('#returns-form').formValid({
			fields: {
				"first_name": {
					"required": true,
					"tests": [
		...
		returnsForm.keypress(300);

		$(document).on("click", '#returns-button', function(e) {
			returnsForm.test();
			e.preventDefault();

			if (returnsForm.errors() > 0) {
				Ladda.stopAll();
			} else {
				var data = {type: 'post', form: '#returns-form', modal: '#modals', doAfter: 'datatable', selector: '#returns', messageError: '.response', showAlert: true, titleResponse: "Exito!", textResponse: "Datos del estudiante actualizados correctamente."};
				DOM.submitData(data);
			}
		});
	});
