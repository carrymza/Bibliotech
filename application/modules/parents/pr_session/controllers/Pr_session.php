...
class Pr_session extends MY_Controller
{
	public function __construct()
	{
		parent::__construct();
		$this->title       = 'Iniciar Sesión';
		$this->load->model('pr_session/pr_session_model');
	...
	public function index($redirect = FALSE)
	{
		...
		$this->load->view('pr_session/pr_session_view', $data);
	}
	...
	public function auth($username  = FALSE, $password  = FALSE)
	{
		...
		$redirect   = (empty($_POST['redirect'])) ? 'pr_dashboard' : $this->input->post('redirect');
		$row        = $this->com_auth->pr_auth( $username, $password);
	...
	public function recover()
	{
		...
		$data           = array();
		$this->load->view('pr_session/pr_recover_view', $data);
	}
	...
	public function recover_password()
	{
		...
		else
		{
			...
			$validate   = $this->session_model->validate_email($email);
			...
			else
			{
				...
				$data['email']  = $email;
				$data['name']   = $name;
				$html           = $this->load->view("pr_session/pr_session_recover_email_view", $data, TRUE);
				...
				echo json_encode(array("result" => 1, 'url' => base_url('pr/session/reset_send')));
			}
		}
	}
	...
	{
		$this->load->view('pr_session/pr_reset_send_view');
	}
	...
	public function new_password($hash = FALSE)
	{
		if($hash != FALSE)
		{
			...
			(!empty($row))? $this->load->view('pr_session/pr_reset_password_view', array('userId' => $row->userId)) : $this->load->view('pr_session/pr_reset_invalid_link_view');
		}
		else
		{
			$this->load->view('pr_session/pr_reset_invalid_link_view');
		}
	}
