...
let loadDatatable = function () {
	let oTable = $('#editorial').DataTable({
		"ajax":{
			url : URL.baseUrl()+'editorial/datatable'
		},
		"columns" : [
...
let options = function (data) {
	...
	html += '<div class="dropdown-primary dropdown open">\n' +
		...
		'<a class="dropdown-item modal_trigger" data-target="#modals" data-toggle="modal" href="javascript:void(0);" data-url="'+URL.baseUrl()+'editorial/edit/'+id+'"><i class="ti-pencil-alt"></i> Editar</a>\n'+
		'<a class="dropdown-item delete_dt_item" data-table="#editorial" data-url="'+URL.baseUrl()+'editorial/delete/'+id+'" href="javascript:void(0);"><i class="ti-trash"></i> Eliminar</a>\n';
	html +='</div></div>';
